...
- Do not modify code while using this skill unless the user also asks for code changes.
- Do not overwrite the PR body with a generic template.
- Do not remove template sections just because they are empty; fill them or leave them for the author when evidence is insufficient.
- If the linked issue or PR cannot be accessed, stop and ask the user for the missing context instead of guessing.
